#!/usr/bin/env python3
# ---------------------------------------------------------------------------
# Vendored from Shubham Saboo's awesome-llm-apps (agent_skills/evals/tools/),
# Apache License 2.0. See ../NOTICE for the full attribution.
#
# Local modifications:
#   * takes a skill directory as an argument
#   * BARE_PATH no longer matches a repo-relative path that merely contains a
#     scripts/ segment, e.g. "video/scripts/build_stage_data.py". The rule is
#     that a SKILL-relative reference must exist; a path rooted elsewhere in the
#     repository is not one, and flagging it made the check unusable for skills
#     that name tools living outside their own directory.
# ---------------------------------------------------------------------------
"""
...
BARE_PATH = re.compile(r"(?<![\w/])((?:scripts|references|assets|evals)/[\w.\-/]+)")
